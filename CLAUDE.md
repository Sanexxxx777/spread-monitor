...
- **DEX без контракта = поиск по ТИКЕРУ + выбор пары ближайшей по цене к другой площадке** (`hint` из движка). Иначе «самый ликвидный» берёт ЧУЖОЙ одноимённый токен (ESPORTS: base $0.072 vs BSC $0.047 → фейк-спред). Контракт = точное совпадение по адресу (приоритет).
