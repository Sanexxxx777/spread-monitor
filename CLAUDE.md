...
- **DEX-форма (`AddCoinDialog`): для DEX поле «Имя» в 1-й строке заменяется на «Контракт токена»** (обязателен), сеть+имя уходят в блок ниже. `dex` = `VENUES[venueA/B].kind==="dex"` (НЕ хардкод имени). При выборе DEX в шапке поле появляется на лету; при ADD-потоке работает, при EDIT см. баг ниже.
...
- **БАГ «Изменить» не загружает значения монеты** (предсуществующий, выявлен 25.06): `AddCoinDialog` всегда смонтирован в `App` (строка 167), `useState(initial?.…)` инициализируется ОДИН раз с `initial=null` → при повторном открытии поля остаются дефолтными (binance/gate, «Имя»), правки монеты теряются. Фикс — `key={dialog.edit?.id ?? "new"}` на `<AddCoinDialog>` (форс-ремоунт) ИЛИ `useEffect`-синк стейта по `initial`. ADD-поток работает (стейт меняется на лету).
