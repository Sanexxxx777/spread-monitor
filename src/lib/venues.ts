...
export const VENUES: Record<VenueId, Venue> = {
  ...
  binance: {
    id: "binance", name: "Binance", color: "#F0B90B", kind: "cex", markets: ["perp", "spot"],
  ...
  bybit: {
    id: "bybit", name: "Bybit", color: "#F7A600", kind: "cex", markets: ["perp", "spot"],
    symbolFor: (b) => `${U(b)}USDT`,
    async quote(_c, s, m) {
      const cat = m === "spot" ? "spot" : "linear";
      const d = await getJSON(`https://api.bybit.com/v5/market/tickers?category=${cat}&symbol=${s}`);
  ...
  okx: {
    id: "okx", name: "OKX", color: "#5BC0FF", kind: "cex", markets: ["perp", "spot"],
    symbolFor: (b, m) => (m === "spot" ? `${U(b)}-USDT` : `${U(b)}-USDT-SWAP`),
    async quote(_c, s, m) {
      const reqs: Promise<any>[] = [getJSON(`https://www.okx.com/api/v5/market/ticker?instId=${s}`)];
      if (m === "perp") reqs.push(getJSON(`https://www.okx.com/api/v5/public/funding-rate?instId=${s}`).catch(() => null));
  ...
  kucoin: {
    id: "kucoin", name: "KuCoin", color: "#01BC8D", kind: "cex", markets: ["perp", "spot"],
    symbolFor: (b, m) => (m === "spot" ? `${U(b)}-USDT` : `${b.toUpperCase() === "BTC" ? "XBT" : U(b)}USDTM`),
    async quote(_c, s, m) {
      if (m === "spot") {
        const d = await getJSON(`https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=${s}`);
        const r = d?.data;
        if (!r) return null;
      ...
      const [t, fr] = await Promise.all([
        ...
        getJSON(`https://api-futures.kucoin.com/api/v1/funding-rate/${s}/current`).catch(() => null),
      ]);
      const r = t?.data;
      if (!r) return null;
      const last = num(r.price);
  ...
  hyperliquid: {
    id: "hyperliquid", name: "Hyperliquid", color: "#50D2C1", kind: "cex", markets: ["perp"],
    symbolFor: (b) => U(b),
    async quote(_c, s) {
  ...
  mexc: {
    id: "mexc", name: "MEXC", color: "#1972F5", kind: "cex", markets: ["perp", "spot"],
    ...
    async quote(_c, s, m) {
      if (m === "spot") {
        ...
        return { last: (bid + ask) / 2, bid, ask, change24: chg !== undefined ? chg / 100 : undefined };
      }
  ...
  gate: {
    id: "gate", name: "Gate", color: "#D06AC0", kind: "cex", markets: ["perp", "spot"],
